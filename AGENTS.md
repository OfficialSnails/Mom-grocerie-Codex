...
- Cloudflare Pages deployment is configured as a static site serving `website/` through `wrangler.jsonc`. Use `npm run cloudflare:login` once, then `npm run deploy:cloudflare`. Current production URL: `https://bons-speciaux-joliette.pages.dev/`. Do not publish `.env`, `.cache/`, `node_modules/`, `output/`, or logs.
- GitHub automation lives in `.github/workflows/weekly-cloudflare.yml`. Once this folder is published to a GitHub repo, add repository secrets `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`, and optionally `FIRECRAWL_API_KEY`; the workflow can regenerate weekly data, commit `data/`, `reports/`, and `website/data/`, then deploy Cloudflare Pages.
...
- The top controls include a store filter. Selecting a store should clear the active rayon and show all good items from that store across categories; clicking a rayon should clear the store filter and return to the category view.
- Rayons should wrap into visible button rows, not a horizontal scroll rail. Keep the week chooser as a custom in-page dropdown so it does not cover the search field like a native select menu.
...
- Category overrides are part of the process: celery/céleri goes in produce; breaded fish and cold cuts go in meat/fish; pizza goes in frozen even when the name contains tomato; bologne/bologna, pepperoni, chorizo, rosette, sauciflard, salami, mortadelle, prosciutto and viandes froides go in meat even if the source category says pantry/epicerie. Deduplicate obvious same-family winners such as bologne, sauciflard/chorizo, and extra-lean ground beef; keep beef rosettes separate from sauciflard/chorizo.
...
- In local mode, `Exporter PDF` should call the Node server and save a clean grocery PDF directly to `~/Desktop`. Do not reduce it back to browser print-only. The PDF should be simple for printing/sharing: store, address, item, and price only, without the website method note or reason columns. A hosted static version cannot silently write to Desktop and will need a download/server flow.
