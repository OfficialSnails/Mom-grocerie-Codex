...
- Before any Cloudflare deploy, scan for unresolved conflict markers (`<<<<<<<`, `=======`, `>>>>>>>`) outside `.git/` and `node_modules/`, then parse `website/data/weeks/index.json` and all `website/data/weeks/*/week.json`. Never deploy a broken generated JSON bundle. If conflict markers appear only in generated grocery artifacts, regenerate clean outputs from the normal pipeline or existing raw data where possible; do not hand-patch generated JSON except as a clearly reported emergency.
...
- The week chooser should keep only a small rolling comparison window of valid production weeks, currently the newest two website weeks, with the newest production week selected by default. Keep older archive folders under `reports/weeks/`, but do not expose every old week in the normal public selector. Hide test/manual-preview/mock weeks from normal users unless debug week mode is explicitly enabled.
...
- Weekly generation targets the real Thursday-to-Wednesday flyer cycle. If the user asks to run/generate the week on Wednesday because new flyers should start Thursday, treat that as the upcoming Thursday cycle, not the Wednesday-ending current calendar cycle. Verify that live Flipp/Wishabi data actually overlaps the generated week and is materially new; do not relabel old products into a future week. If `npm run weekly` would still generate the old cycle because today's date is Wednesday, use the same normal pipeline with `BONS_SPECIAUX_RUN_DATE=<THURSDAY_ISO_DATE> npm run weekly` for the upcoming Thursday and report the override.
- `data/current_week_prices.csv` is only a dated manual supplement. Rows must overlap the generated flyer cycle through `week_start_date`/`week_end_date`; expired or undated manual rows must be skipped by CSV collection, report generation, website export, and history updates. Never let an old manual CSV price become a recurring current-week product.
...
- Category overrides are part of the process: celery/céleri, clémentines, pitaya, poires, dattes, romaine, prepared fruit/vegetable trays, crudités, maïs en épi and maïs sucré go in produce; breaded fish, goberge, homard, brochettes, côtelettes, veal/veau, surlonge and cold cuts go in meat/fish; pizza, Eggo waffles, frozen desserts and frozen meals go in frozen when clearly frozen; sunscreen, dental care, digestive/allergy/meal-replacement pharmacy items and cosmetics go in health; detergent, dish soap, paper goods, diapers and laundry products go in household. Bologne/bologna, pepperoni, chorizo, rosette, sauciflard, salami, mortadelle, prosciutto and viandes froides go in meat even if the source category says pantry/epicerie. Keep `Garde-manger et autres` as the visible pantry fallback after stronger rules, and run `npm run qa:pantry` plus `npm run qa:categories` after classifier changes. Deduplicate obvious same-family winners such as bologne, sauciflard/chorizo, and extra-lean ground beef; keep beef rosettes separate from sauciflard/chorizo.
...
- Visible item details should stay useful and compact: hide filler reasons like `Premier aperçu retenu` and `Bon prix si tu en as besoin`; keep concrete savings, percent differences, and cross-store comparisons. Use proof-photo OCR when available in two ways: first, recover high-confidence visual offers that Flipp/Wishabi may miss from complex flyer tiles, then recover obvious `/100g`, `/lb`, or `/kg` units missing from Flipp metadata. OCR-recovered offers must be appended before scoring so they flow through `Tous les produits`, category classification, dedupe, reports and website JSON automatically. Do not patch generated JSON by hand. For unknown units, use `Format à vérifier sur la photo.` when a proof image exists, or `Format non confirmé.` without an image.
- OCR-recovered offers need proof-image validation before publishing. The attached proof must contain the recovered product keywords and price/rebate context; if the product is visible only on a wider flyer page, create or use a source-backed crop around the OCR match. Never publish a nearby but unrelated proof photo. A missing proof image is safer than a wrong proof image.
- In local mode, `Exporter PDF` should call the Node server and save a clean grocery PDF directly to `~/Desktop`. Do not reduce it back to browser print-only. The PDF should be simple for printing/sharing: store, address, item, price, notes, a safe `Total estimé` near the top, store-level subtotals, and `Total estimé de la liste` at the end, without the website method note or reason columns. On the hosted static Cloudflare version, use the browser-side PDF download flow so Windows/Mac users get a real `.pdf` file without needing the local Node server. Keep the adjacent `Partager` button: generate the same PDF, use the native Web Share file sheet when available on mobile, and fall back to a normal download when sharing is unavailable.
