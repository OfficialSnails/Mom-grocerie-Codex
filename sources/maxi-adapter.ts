...
export class MaxiAdapter extends FirecrawlBaseAdapter implements SourceAdapter {
  ...
  async collect(): Promise<RawDealItem[]> {
    ...
    const items = await this.scrapeAsJSON<MaxiFlyerItem>(this.flyer_url, JSON_PROMPT);
    if (!items) return [];
