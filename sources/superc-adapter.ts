...
export class SuperCAdapter extends FirecrawlBaseAdapter implements SourceAdapter {
  ...
  async collect(): Promise<RawDealItem[]> {
    ...
    const items = await this.scrapeAsJSON<SuperCFlyerItem>(this.flyer_url, JSON_PROMPT);
    if (!items) return [];
